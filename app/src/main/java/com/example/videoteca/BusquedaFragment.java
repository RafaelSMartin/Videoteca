...
public class BusquedaFragment
        extends android.support.v17.leanback.app.SearchFragment implements
    ...
    private void loadQuery(String query) {
        mQuery = query; mHandler.removeCallbacks(mDelayedLoad);
    ...
    private void loadRows() {
        ...
        for( Movie movie : MovieList.list ) {
            if (movie.getTitle().toLowerCase(Locale.ENGLISH)
                    .contains(mQuery.toLowerCase(Locale.ENGLISH)) || movie.getDescription().toLowerCase(Locale.ENGLISH)
                    .contains(mQuery.toLowerCase(Locale.ENGLISH))) { i=i+1; listRowAdapter.add(movie);
            }
        }
