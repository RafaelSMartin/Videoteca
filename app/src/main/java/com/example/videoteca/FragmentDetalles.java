...
//            selectedIndex = Integer.parseInt(url.getQueryParameter("id"));
            if (url.getQueryParameter("id")==null){
                selectedIndex = Integer.parseInt(getActivity().getIntent()
                        .getData().getLastPathSegment());
            } else {
                selectedIndex = Integer.parseInt(url.getQueryParameter("id"));
            }
            mSelectedMovie = MovieList.list.get(selectedIndex);
        } else {
            mSelectedMovie = (Movie) getActivity().getIntent()
    ...
    private void setupDetailsOverviewRow() {
        ...
        row.setImageDrawable(ContextCompat.getDrawable(getContext(),
                R.drawable.default_background));
        int width = convertDpToPixel(getActivity().getApplicationContext(),
                DETAIL_THUMB_WIDTH);
        int height = convertDpToPixel(getActivity().getApplicationContext(),
                DETAIL_THUMB_HEIGHT);
    ...
    private void setupDetailsOverviewRowPresenter() {
        ...
        sharedElementHelper.setSharedElementEnterTransition(
                getActivity(), ActividadDetalles.SHARED_ELEMENT_NAME);
                ...
                new OnActionClickedListener() {
                    ...
                    public void onActionClicked(Action action) {
                        if (action.getId() == ACTION_VER_TRAILER) {
                            ...
                            Toast.makeText(getActivity(), action.toString(),
